...
contract ICYMigration is Ownable {
    ...
    function migrate() external onlyInitialized {
        uint256 amountGLA = GLA.balanceOf(msg.sender);
        uint256 amountICE = ICE.balanceOf(msg.sender);
        require(startTimestamp < block.timestamp, "Not started yet");
        require(
            block.timestamp < endTimestamp,
            "swapping of GLA and ICE has ended"
        );
        if (amountGLA > 0) {
            GLA.transferFrom(msg.sender, address(this), amountGLA);
            ICY.transfer(msg.sender, amountGLA.div(100));
        }
        if (amountICE > 0) {
            ICE.transferFrom(msg.sender, address(this), amountICE);
            ICY.transfer(msg.sender, amountICE.div(100));
        }
    }
    ...
    function withdraw(address token) external onlyOwner {
        require(
            block.timestamp > endTimestamp,
            "swapping of GLA or ICE has not ended"
        );
