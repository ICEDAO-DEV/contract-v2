...
contract DepositHelper is Ownable {
    ...
    constructor (address _deposit, ISwapV2Router _router, address _principle, address _ICY, address _MIN, ITreasury _treasury) {
    ...
    function depositHelper(
        ...
        if (_tokenAddress == principle) {
            ...
            ICY.safeApprove(address(router), uint256(- 1));
            ...
            (,, uint256 moreLPAmount) = router.addLiquidity(MIN, ICY, MIN.myBalance(), ICY.myBalance(), 0, 0, address(this), block.timestamp);
            principle.safeApprove(address(deposit), moreLPAmount);
            if (MIN.myBalance() > 0) {
                MIN.safeTransfer(msg.sender, MIN.myBalance());
            }
            if (ICY.myBalance() > 0) {
                ICY.safeTransfer(msg.sender, ICY.myBalance());
            }
    ...
    function depositValue(uint256 _amount) public view returns (uint256 value_) {
        ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(MIN, ICY));
        ...
        if (swapAmt > 0) {
            ...
            (path[0], path[1]) = isReversed ? (ICY, MIN) : (MIN, ICY);
