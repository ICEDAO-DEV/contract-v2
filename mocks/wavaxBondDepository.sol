...
contract wavaxBondDepository is Ownable {
    ...
    address public immutable ICY; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints ICY when receives principle
    address public immutable DAO; // receives profit share from bond
    address public immutable v2Router;
    address public immutable mim_wavax_lp;
    ...
    struct Bond {
        uint256 payout; // ICY remaining to be paid
    ...
    constructor(
        ...
        address _v2Router,
        address _mim_wavax_lp
    ) {
        require(_ICY != address(0));
        ICY = _ICY;
    ...
    function deposit(
        ...
        // must be > 0.0001 ICY ( underflow protection )
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(ICY).transfer(_recipient, _amount);
            // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(ICY).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(ICY).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(ICY).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function bondPrice() public view returns (uint256 price_) {
        price_ = terms.controlVariable.mul(debtRatio()).div(1e7); //1e7 1e7
     ...
     */
    function assetPrice() public view returns (uint256 result) {
        (uint256 reserve0, uint256 reserve1, ) = IPancakePair(mim_wavax_lp)
            .getReserves();
        result = principle == IPancakePair(mim_wavax_lp).token1()
            ? reserve0.mul(1e18).div(reserve1)
            : reserve1.mul(1e18).div(reserve0);
    }
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        price_ = bondPrice().mul(uint256(assetPrice())).div(100);
    }

    /**
     *  @notice calculate current ratio of debt to ICY supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(ICY).totalSupply();
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        return debtRatio().mul(uint256(assetPrice())).div(1e18); // ETH feed is 8 decimals
    }
    /**
    ...
    /**
     *  @notice calculate amount of ICY available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or ICY) to the DAO
     *  @return bool
     */
